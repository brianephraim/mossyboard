...
vi.mock("react-virtuoso", () => {
  type ItemContent = (index: number, item: unknown) => unknown;
  type Components = { Footer?: () => unknown };
  ...
  const Virtuoso = (props: VirtuosoProps) => {
    ...
    const renderedItems = items.map((item, index) =>
      createElement(
        Fragment,
        { key: index },
        props.itemContent ? (props.itemContent(index, item) as React.ReactNode) : null,
      ),
    );
